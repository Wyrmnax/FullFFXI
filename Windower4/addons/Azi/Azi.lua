...
tp = false
affi = false
runNextLua = true -- used to make commands not repeat themselves [Spammy commands]
weaponSwitch = true -- used to make commands not repeat themselves [Spammy commands]
pause = 'on'
...
default = {
	...
	start_fight_commands = 'gs c set AutoBuffMode On;gs c set AutoWSMode On;gs c autows Savage Blade;gs c autows tp 2000',
...
function fight()
	...
	elseif not isBuffActive(603) and not isBuffActive(2) then			--If our elvorseal is gone, try to take a raise if we're dead, or release if we don't have one. teleport ring out
		...
		if player.status > 1 then
			...
			-- Advised to bring Warp Club, BLM Warp. Warp ring may not be able to keep up!!
				
				coroutine.sleep(10)
				if weaponSwitch == true then
				windower.send_command('input //gs c Weapons none;wait 3; input /warp')
				weaponSwitch = false
				end
				delay = 60
				if runNextLua == true then
					windower.send_command('wait 3;input //lua l naga;wait 5;input //naga start;wait 5;input //lua u azi')
					runNextLua = false	
				end
			delay = 30
		end
