...
function midcast(spell,arg)
	...
	--windower.add_to_chat(121, "Equip helix " )
...
-- Elemental Magic
	...
	elseif spell.skill == 'Elemental Magic' then	
		...
		elseif windower.wc_match(spell.name, 'Frost|Drown|Rasp|Burn|Shock|Choke') then
			equip(sets.midcast.Elemental)
		elseif windower.wc_match(spell.name, 'Fire|Stone|Aero|Blizzard|Thunder|Water|Dia') then
			equip(sets.midcast.LowNuke)			
		elseif windower.wc_match(spell.name, '*helix*') then
			equip(sets.midcast.Helix)			
			--if spell.element == world.weather_element or spell.element == world.day_element or spell.element == buffactive[elements.storm_of[spell.element]] then				
				--windower.add_to_chat(121,'Obi' ..spell.skill)
