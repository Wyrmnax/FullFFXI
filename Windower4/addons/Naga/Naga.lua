...
tp = false
dremi = false
qufimZone = true -- used to make commands not repeat themselves [Spammy commands]
warp = true -- used to make commands not repeat themselves [Spammy commands]
pause = 'on'
...
windower.register_event('prerender', function()
	...
	if nexttime + delay <= curtime then
		...
		elseif zone == 'Qufim Island' then
			if qufimZone == true then
				coroutine.sleep(15)
				if warp == true then
				windower.send_command('input //hp Misareaux Coast')
				warp = false
				end
			end
			qufimZone = false
	...
	elseif not isBuffActive(603) and not isBuffActive(2) then			--If our elvorseal is gone, try to take a raise if we're dead, or release if we don't have one. teleport ring out
		...
		if player.status > 1 then
			...
			if getEquippedItem('right_ring') ~= settings.teleport_ring then
				...
				windower.send_command('wait 3;input /item "'..settings.teleport_ring..'" <me>;wait 3;gs enable ring2; wait 30;')
				delay = 120
				windower.send_command('input //lua l quetz;wait 5;input //quetz start;wait 3;input //lua u naga')
			end
			delay = 30
		end
	end
