...
--[[trust1 = 'Yoran-Oran (UC)'
trust1_short = 'Yoran Oran'
trust2 = 'Selh\'teus'
trust2_short = 'Selh\'teus'
trust3 = 'Joachim'
trust3_short = 'Joachim'
trust4 = 'Ulmia'
trust4_short = 'Ulmia'
trust5 = 'Koru-Moru'
...
default = {
	...
	start_fight_commands = 'gs c set AutoBuffMode On;gs c set AutoWSMode On;gs c autows Savage Blade',
	...
	if nexttime + delay <= curtime then
		...
		elseif zone ~= 'Reisenjima' and zone ~= 'La Theine Plateau' and zone ~= 'Konschtat Highlands' and zone ~= 'Tahrongi Canyon' then	--If we were sent to our home point for any reason, just warp our ass back out there
			if getEquippedItem('right_ring') ~= settings.teleport_ring then
				windower.send_command('input /equip ring2 "'..settings.teleport_ring..'"')
			else
				windower.send_command('input /item "'..settings.teleport_ring..'" <me>')
				delay = 30
				log('Next is Azi')
				windower.send_command('input //lua l azi;wait 5;input //azi start; wait 5; input //lua u quetz')
			end
			delay = 15
		end		
	end
...
function findWaitPosition()
	wait_x = settings.quetz_x + math.random(0.10,1.5)
	wait_y = settings.quetz_y + math.random(0.10,1.5)
end
...
function fight()
	...
	elseif not isBuffActive(603) and not isBuffActive(2) then			--If our elvorseal is gone, try to take a raise if we're dead, or release if we don't have one. teleport ring out
		...
		if player.status > 1 then
			...
			if getEquippedItem('right_ring') ~= settings.teleport_ring then
				windower.send_command('wait 3;input /equip ring2 "'..settings.teleport_ring..'"')
			else
				windower.send_command('wait 3;input /item "'..settings.teleport_ring..'" <me>')
			end
			delay = 30
		end
	end
...
windower.register_event('incoming chunk',function(id,data,modified,injected,blocked)
	...
	elseif id == 0x036 and not isBuffActive(603) then
		...
		if gob and npc_id == gob.id and message_id == 6407 then
			delay = 60
		end
	end
end)
...
windower.register_event('addon command', function(...)
    ...
    elseif command[1] == 'start' then
		pause = 'off'
		log('Starting Quetz')
		windower.send_command('input //gs c Weapons Kaja;')
	end
end)